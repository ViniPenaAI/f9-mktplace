...
interface ConfiguratorState {
    ...
    /** true quando o usuário entrou no pagamento pelo carrinho (?checkout=1) */
    checkoutFromCart: boolean;

    // Actions
    setStep: (step: number) => void;
    setCheckoutFromCart: (v: boolean) => void;
    ...
    calculatePrice: () => void;
    /** Zera todas as configurações e volta ao passo 1 (ex.: ao clicar em Início). */
    reset: () => void;
}
...
export const useConfiguratorStore = create<ConfiguratorState>((set, get) => ({
    ...
    setCheckoutFromCart: (v) => set({ checkoutFromCart: v }),
    ...
    reset: () =>
        set({
            ...
            customer: { name: '', email: '', phone: '', document: '', documentType: 'cpf', razao_social: '', phone_business: '', email_business: '' },
            shipping: { zipCode: '', street: '', number: '', city: '', state: '' },
            totalPrice: 0,
            shippingCost: 0,
            selectedShippingOption: null,
            shippingInsurance: 'with',
